...
xlabel(::GradHist1dQuantity) = "Gradient element value"
...
function _objects_to_panels(objs::Vector{<:AbstractQuantity})
    ...
    grad_priority(o) = o isa NormTestQuantity ? 0 : o isa GradHist1dQuantity ? 1 : 2
...
function _create_group!(cell, n_objs, title; overlay = false, two_cols = false)
    ...
    Label(cell[1, 1:3], title; fontsize = 13, color = TX_MAIN, valign = :center)
    rowgap!(cell, 6)
    colgap!(cell, 6)
...
function build_dashboard(qs::Vector{<:AbstractQuantity})
    ...
    Label(
        gd[1, 1:3, Top()],
        "Live Monitoring of ML Training",
        fontsize = 20,
        color = TX_MAIN,
    )
    ...
    if length(panels) == 1
        ...
        rowsize!(wrap, 1, Relative(0.02))
        rowsize!(wrap, 2, Relative(0.96))
        rowsize!(wrap, 3, Relative(0.02))
        colsize!(wrap, 1, Relative(0.02))
        colsize!(wrap, 2, Relative(0.96))
        colsize!(wrap, 3, Relative(0.02))
        ...
        rowsize!(wrap1, 1, Relative(0.02))
        rowsize!(wrap1, 2, Relative(0.96))
        rowsize!(wrap1, 3, Relative(0.02))
        colsize!(wrap1, 1, Relative(0.02))
        colsize!(wrap1, 2, Relative(0.96))
        colsize!(wrap1, 3, Relative(0.02))

        rowsize!(wrap2, 1, Relative(0.02))
        rowsize!(wrap2, 2, Relative(0.96))
        rowsize!(wrap2, 3, Relative(0.02))
        colsize!(wrap2, 1, Relative(0.02))
        colsize!(wrap2, 2, Relative(0.96))
        colsize!(wrap2, 3, Relative(0.02))
    ...
    for (gp, (cls, items)) in zip(gridlayouts, panels)
        ...
        ax1, ax2, ax_overlay =
            _create_group!(gp, n_objs, title; overlay = needs_overlay, two_cols = two_cols)
        ...
        if n_objs == 1
            ...
            _label_axis!(
                ...
                ax1,
                items[1],
                (items[1] isa CombinedQuantity) ? ax_overlay : nothing,
            )
            if xlabel(items[1]) == "Iteration"
                push!(iteration_axes, ax1)
            end
